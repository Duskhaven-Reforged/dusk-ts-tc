...
{
    if (HasAuraType(SPELL_AURA_MONK_UNARMED))
        return true;
    ...
    if (damagetype != NODAMAGE)
    {
        ...
        if (spellProto)
        {
            if (!spellProto->HasAttribute(SPELL_ATTR4_DAMAGE_DOESNT_BREAK_AURAS))
                victim->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_TAKE_DAMAGE, spellProto->Id, attacker, damagetype);
        }
        else
            victim->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_TAKE_DAMAGE, 0, attacker, damagetype);
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(Unit const* victim, WeaponAttackType attType) const
{
    ...
    // @tswow-begin
    float miss_chance_f = MeleeSpellMissChance(victim, attType, 0, 0);
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spellInfo) const
{
    ...
    uint32 missChance = uint32(MeleeSpellMissChance(victim, attType, 0, spellInfo->Id) * 100.0f);
    // Roll miss
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, SpellEffectInfo const& spellEffectInfo, Optional<float> const& donePctTotal, uint32 stack /*= 1*/) const
{
    ...
    if (SpellBonusEntry const* bonus = sSpellMgr->GetSpellBonusData(spellProto->Id, spellEffectInfo.EffectIndex))
    {
        WeaponAttackType const attType =
            (spellProto->IsRangedWeaponSpell() && spellProto->DmgClass != SPELL_DAMAGE_CLASS_MELEE) ? RANGED_ATTACK : spellProto->HasAttribute(SPELL_ATTR3_MAIN_HAND) ? BASE_ATTACK : OFF_ATTACK;
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype) const
{
    ...
    if (!spellProto->HasAttribute(SPELL_ATTR4_FIXED_DAMAGE))
    {
        ...
        if (caster)
        {
            TakenTotalMod *= GetTotalAuraMultiplier(SPELL_AURA_MOD_DAMAGE_FROM_CASTER_SPELL, [caster, spellProto](AuraEffect const* aurEff) -> bool
                {
...
/*static*/ uint32 Unit::SpellCriticalDamageBonus(Unit const* caster, SpellInfo const* spellProto, uint32 damage, Unit* victim)
{
    ...
    if (caster)
    {
        ...
        case SPELL_DAMAGE_CLASS_MELEE:                      // for melee based spells is 100%
            crit_bonus = damage;
            crit_mult += caster->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_BASE_CRIT_DAMAGE, SPELL_DAMAGE_CLASS_MASK_MELEE);
            break;
        case SPELL_DAMAGE_CLASS_RANGED:
            crit_bonus = damage;
            /// @todo write here full calculation for melee/ranged spells
            crit_mult += caster->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_BASE_CRIT_DAMAGE, SPELL_DAMAGE_CLASS_MASK_RANGED);
            break;
        default: // for spells is 50%
            crit_bonus = damage / 2;
            crit_mult += caster->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_BASE_CRIT_DAMAGE, SPELL_DAMAGE_CLASS_MASK_MAGIC);
            break;
        }

        crit_mult += (caster->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_CRIT_DAMAGE_BONUS, spellProto->GetSchoolMask()) - 1.0f) * 100;

        if (victim)
            crit_mult += caster->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_CRIT_PERCENT_VERSUS, victim->GetCreatureTypeMask());

        if (caster->IsPlayer())
            FIRE(Player, OnCustomScriptedCritDamageMod, TSPlayer(const_cast<Player*>(caster->ToPlayer())), TSUnit(victim), TSSpellInfo(const_cast<SpellInfo*>(spellProto)), TSMutableNumber<float>(&crit_mult));

        if (crit_mult != 0)
            AddPct(crit_bonus, crit_mult);
...
/*static*/ uint32 Unit::SpellCriticalHealingBonus(Unit const* caster, SpellInfo const* spellProto, uint32 damage, Unit* victim)
{
    ...
    if (caster)
    {
        ...
        if (victim)
            crit_mult += caster->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_CRIT_PERCENT_VERSUS, victim->GetCreatureTypeMask());
...
uint32 Unit::SpellHealingBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype) const
{
    ...
    if (caster)
    {
        ...
        TakenTotalMod *= GetTotalAuraMultiplier(SPELL_AURA_MOD_SCHOOL_MASK_HEALING_FROM_CASTER, [caster, spellProto](AuraEffect const* aurEff) -> bool
            {
                if (caster->GetGUID() == aurEff->GetCasterGUID() && aurEff->IsAffectedOnSpell(spellProto) && (!aurEff->GetMiscValue() || (aurEff->GetMiscValue() & spellProto->GetSchoolMask()) != 0))
                    return true;
                return false;
            });
    }
...
uint32 Unit::MeleeDamageBonusTaken(Unit* attacker, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto /*= nullptr*/, SpellSchoolMask damageSchoolMask /*= SPELL_SCHOOL_MASK_NORMAL*/)
{
    ...
    {
        // From caster spells
        TakenTotalMod *= GetTotalAuraMultiplier(SPELL_AURA_MOD_DAMAGE_FROM_CASTER_SPELL, [attacker, spellProto](AuraEffect const* aurEff) -> bool
        {
        ...
        if (attacker->IsPlayer())
            FIRE_ID(spellProto->events.id, Spell, OnCustomMechanicMaskDamage, TSUnit(attacker), TSSpellInfo(spellProto), TSMutableNumber<uint32>(&mechanicMask));
...
{
    if ((att == BASE_ATTACK || att == OFF_ATTACK) && HasAuraType(SPELL_AURA_MONK_UNARMED))
