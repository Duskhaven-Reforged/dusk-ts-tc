...
{
    UpdateDelayMomentForDst(CalculateDelayMomentForDst());
}
...
void Spell::UpdateDelayMomentForUnitTarget(Unit* unit, uint64 hitDelay)
{
    ...
    {
        auto minDelayTargetItr = std::min_element(m_UniqueTargetInfo.begin(), m_UniqueTargetInfo.end(), [](Spell::TargetInfo const& targetInfo, Spell::TargetInfo const& smallest)
        {
            return targetInfo.TimeDelay && targetInfo.TimeDelay < smallest.TimeDelay;
        });

        m_delayMoment = minDelayTargetItr != m_UniqueTargetInfo.end() ? minDelayTargetItr->TimeDelay : 0;
    }
...
void Spell::SelectImplicitNearbyTargets(SpellEffectInfo const& spellEffectInfo, SpellImplicitTargetInfo const& targetType, uint32 effMask, uint32 WithoutAura /* = 0 */)
{
    ...
    switch (targetType.GetCheckType())
    {
        ...
        case TARGET_CHECK_SUMMONER_ALLY:
...
void Spell::SelectImplicitConeTargets(SpellEffectInfo const& spellEffectInfo, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (uint32 containerTypeMask = GetSearcherTypeMask(objectType, condList))
    {
        ...
        Trinity::WorldObjectSpellConeTargetCheck check(coneSrc, coneAngle, m_caster->GetCombatReach(), radius, m_caster, m_spellInfo, selectionType, condList, objectType);
        ...
        case TARGET_UNIT_TARGET_ALLY_OR_RAID:
            if (Unit* targetedUnit = m_targets.GetUnitTarget())
            {
                if (!m_caster->IsUnit() || !m_caster->ToUnit()->IsInRaidWith(targetedUnit))
                    ...
                    SearchAreaTargets(targets, radius, targetedUnit, referer, targetType.GetObjectType(), TARGET_CHECK_RAID,
                        spellEffectInfo.ImplicitTargetConditions);
            }
            break;
        default:
...
void Spell::SelectImplicitTrajTargets(SpellEffectInfo const& spellEffectInfo, SpellImplicitTargetInfo const& targetType)
{
    ...
    std::list<WorldObject*> targets;
    Trinity::WorldObjectSpellTrajTargetCheck check(dist2d, &srcPos, m_caster, m_spellInfo, targetType.GetCheckType(), spellEffectInfo.ImplicitTargetConditions, targetType.GetObjectType());
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionContainer* condList, uint32 WithoutAura)
{
    ...
    if (!containerTypeMask)
        return nullptr;
    if (WithoutAura) {
        Trinity::WorldObjectSpellNearbyTargetWithoutAuraCheck check(range, m_caster, m_spellInfo, selectionType, condList, objectType, WithoutAura);
...
void Spell::SearchChainTargets(std::list<WorldObject*>& targets, uint32 chainTargets, WorldObject* target, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectType, SpellEffectInfo const& effectInfo, bool isChainHeal)
{
    // max dist for jump target selection
    float jumpRadius = effectInfo.CalcRadius(GetCaster(), this);
    switch (m_spellInfo->DmgClass)
    {
        case SPELL_DAMAGE_CLASS_RANGED:
            // 7.5y for multi shot
    ...
    SearchAreaTargets(tempTargets, searchRadius, chainSource, m_caster, objectType, selectType, effectInfo.ImplicitTargetConditions);
    ...
    if (unitTarget)
    {
        ...
        switch (_targetSelectionType)
        {
            ...
            case TARGET_CHECK_ALLY:
                ...
                break;
            case TARGET_CHECK_SUMMONER_ALLY:
                if (auto owner = _caster->GetOwner()) {
                    if (unitTarget->IsTotem())
                        return false;

                    if (!target->IsCorpse() && !owner->IsValidAssistTarget(unitTarget, _spellInfo))
                        return false;
                } else
                    return false;
                break;
...
WorldObjectSpellNearbyTargetWithoutAuraCheck::WorldObjectSpellNearbyTargetWithoutAuraCheck(float range, WorldObject* caster, SpellInfo const* spellInfo, SpellTargetCheckTypes selectionType,
    ConditionContainer const* condList, SpellTargetObjectTypes objectType, uint32 WithoutAura)
    : WorldObjectSpellTargetCheck(caster, caster, spellInfo, selectionType, condList, objectType), _range(range), _position(caster), _AuraCheck(WithoutAura) { }
...
WorldObjectSpellAreaTargetCheck::WorldObjectSpellAreaTargetCheck(float range, Position const* position, WorldObject* caster, WorldObject* referer, SpellInfo const* spellInfo,
    SpellTargetCheckTypes selectionType, ConditionContainer const* condList, SpellTargetObjectTypes objectType,
...
: WorldObjectSpellAreaTargetCheck(range, caster, caster, caster, spellInfo, selectionType, condList, objectType), _coneAngle(coneAngle), _coneSrc(coneSrc), _lineWidth(lineWidth)
{
}
