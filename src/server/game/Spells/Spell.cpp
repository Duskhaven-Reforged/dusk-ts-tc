...
void Spell::SearchChainTargets(std::list<WorldObject*>& targets, uint32 chainTargets, WorldObject* target, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectType, SpellEffectInfo const& effectInfo, bool isChainHeal)
{
    ...
    SearchAreaTargets(tempTargets, searchRadius, target, m_caster, objectType, selectType, effectInfo.ImplicitTargetConditions);
...
void Spell::DoSpellEffectHit(Unit* unit, SpellEffectInfo const& spellEffectInfo, TargetInfo& hitInfo)
{
    if (uint8 aura_effmask = Aura::BuildEffectMaskForOwner(m_spellInfo, 1 << spellEffectInfo.EffectIndex, unit))
    {
        ...
        if (caster)
        {
            ...
            if (!hitInfo.HitAura)
            {
                bool const resetPeriodicTimer = (m_spellInfo->StackAmount < 2) && !(_triggeredCastFlags & TRIGGERED_DONT_RESET_PERIODIC_TIMER) && !m_spellInfo->HasAttribute(SPELL_ATTR0_CU_DONT_RESTART_P_TIMER);
