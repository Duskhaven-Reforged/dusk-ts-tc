    ...
    if (result)
    {
        do
        {
            ...
            while (spellInfo)
            {
                ...
                if (procEntry.SpellFamilyName && (procEntry.SpellFamilyName < SPELLFAMILY_MAGE || procEntry.SpellFamilyName > SPELLFAMILY_BARD || procEntry.SpellFamilyName == 14 || procEntry.SpellFamilyName == 16))
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i)
    {
        ...
        for (SpellEffectInfo& spellEffectInfo : spellInfo->_GetEffects())
        {
            switch (spellEffectInfo.Effect)
            {
                ...
                case SPELL_EFFECT_JUMP_DEST:
                case SPELL_EFFECT_LEAP_BACK:
                    if (!spellInfo->Speed && !spellInfo->SpellFamilyName && !spellInfo->HasAttribute(SPELL_ATTR1_CU_MISSILE_SPEED_IS_DELAY_IN_SEC))
