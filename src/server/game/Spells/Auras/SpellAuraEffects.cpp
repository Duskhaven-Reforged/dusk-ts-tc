...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //181 SPELL_AURA_MOD_COOLDOWN_BY_HASTE_REGEN
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //198 SPELL_AURA_MOD_GLOBAL_COOLDOWN_BY_HASTE_REGEN
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //222 SPELL_AURA_MOD_DAMAGE_FROM_CASTER
    &AuraEffect::HandleNoImmediateEffect,                         //223 SPELL_AURA_RAID_PROC_FROM_CHARGE
    &AuraEffect::HandleNoImmediateEffect,                         //224 unused (3.0.8a)
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //271 SPELL_AURA_MOD_DAMAGE_FROM_CASTER_SPELL    implemented in Unit::SpellDamageBonus
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //302 SPELL_AURA_MOD_CHANGE_DAMAGE_SCHOOL_OF_SPELL
...
int32 AuraEffect::CalculateAmount(Unit* caster)
{
    ...
    if (!m_spellInfo->HasAttribute(SPELL_ATTR1_CU_STACKS_DONT_ADDUP))
            ...
            switch (form)
            {
                ...
                case FORM_DIREBEAR:
                {
                    switch (GetMiscValue())
                    {
                        case FORM_BEAR:
                        case FORM_DIREBEAR:
                            target->SetPower(POWER_RAGE, 250);
                            break;
                        case FORM_CAT:
                        default:
                            break;
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    uint32 stackAmountForBonuses = !m_spellInfo->HasAttribute(SPELL_ATTR1_CU_STACKS_DONT_ADDUP) ? GetBase()->GetStackAmount() : 1;
    // ignore negative values (can be result apply spellmods to aura damage
    uint32 damage = std::max(GetAmount(), 0);
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    else
    {
        // dynobj auras must always have a caster
        if (GetBase()->GetType() == DYNOBJ_AURA_TYPE)
            damage = ASSERT_NOTNULL(caster)->SpellHealingBonusDone(target, GetSpellInfo(), damage, DOT, GetSpellEffectInfo(), { }, stackAmountForBonuses);
    }
