...
LEFT JOIN `creature_template` ct ON ct.`entry` = cta.`entry`
WHERE ct.`entry` IS NULL;

DELETE cet
FROM `creature_equip_template` cet
LEFT JOIN `creature_template` ct ON ct.`entry` = cet.`CreatureID`
WHERE ct.`entry` IS NULL;
...
UPDATE `creature_template` base
JOIN `creature_template` diff
